...
# -*- coding: utf-8 -*-
"""
SEC-AUDITD Alert Engine - Python 2.7 Compatible
...
class RuleEngine(object):
    ...
    def _eval_filter(self, expr, event):
        """评估过滤表达式（安全实现）"""
        try:
            # 创建安全的上下文
            context = {}
            for key, value in event.items():
                if isinstance(value, (str, int, float, bool, unicode)):
                    context[key] = value
    ...
    def _safe_eval_fallback(self, expr, context):
    ...
    def _eval_ast_node(self, node, context):
        ...
        # Python 2.7 compatibility - use old AST node types
        if isinstance(node, ast.Str):
            return node.s
        if isinstance(node, ast.Num):
            return node.n
        if isinstance(node, ast.Name):
            if hasattr(node.ctx, '__class__') and node.ctx.__class__.__name__ == 'Load':
    ...
    def _check_aggregate(self, event, rule):
    ...
    def _in_whitelist(self, event, rule):
        """检查是否在白名单"""
    ...
    def _generate_alert(self, event, rule, extra_ctx=None):
        """生成告警"""
    ...
    def _should_throttle(self, alert, rule):
        """检查告警是否应该被限流"""
...
class AlertEngine(object):
    ...
    def _run_file_mode(self, filepath):
        """文件模式：读取日志文件"""
    ...
    def _output_to_file(self, alert, config):
        """输出到文件"""
